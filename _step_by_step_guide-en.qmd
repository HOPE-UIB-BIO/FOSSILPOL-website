...
### [I. Data sourcing]{.text-background-blue .bold}: `/01_Neotoma_source/`

- `Run_01_01.R`- run all scripts within this folder

- [`01_Download_neotoma.R`](#download_neotoma.r)- download the pollen data from the Neotoma database

- [`02_Extract_samples.R`](#extract_samples.r) - create a table from Neotoma's downloaded lists and download author information

- [`03_Filter_dep_env.R`](#filter_dep_env.r) - get depositional environment data and filter out records based on the user preferences

- [`04_Extract_chron_control_tables.R`](#extract_chron_control_tables.r) - get chronologies, including the preferred table with chronology control points

- [`05_Extract_raw_pollen_data.R`](#extract_raw_pollen_data.r) - extract the raw pollen counts from Neotoma and filter by user-selected ecological groups.
...
### [II. Data sourcing]{.text-background-blue .bold}: `/02_Other_source/`

- `Run_01_02.R` - run all scripts within this folder
- [`01_Import_other_data.R`](#import_other_data.r) - source other data sources and filter the records in a similar way as Neotoma.

#### Note on other data sourcing
...
3. Names of data contributors are extracted and added to the [Author-Dataset database](#extract_samples.r) used in Author-dataset attribution (see [section VII - Outputs](#vii.-outputs-07_outputs)).
...
### [III. Initial data processing]{.text-background-green .bold}: `/03_Merging_and_geographic_delineation/`

- `Run_01_03.R` - run all scripts within this folder
- [`01_Merge_datasets.R`](#merge_datasets.r) - merge data from all sources, filter out duplicates, and assign values based on geographical location
...
1. All taxon names are transformed into a more computer-friendly format for easier manipulation. The `RFossilpol::proc_clean_count_names()` function will first transform special characters to text (e.g. `+` to `_plus_`) and then use the {[janitor](https://sfirke.github.io/janitor/)} to transform into ["*snake_case*" style](https://en.wikipedia.org/wiki/Snake_case). In addition, the user can specify additional specific changes in names (e.g. based on presence of special characters) by adjusting the `user_name_patterns` argument (see example in the script). During this cleaning of taxa names, the Workflow will save `taxa_refrence_table` for back traceability to Neotoma taxonomy. The `taxa_reference_table` is a CSV file saved in the same folder as the harmonisation tables (`Data/Input/Harmonisation_tables/`). More about [harmonisation process](#v.-harmonisation-05_harmonisation).
...
### [IV. Chronologies]{.text-background-purple .bold}: `/04_Chronologies/`

- `Run_01_04.R` - run all scripts within this folder
- [`01_Prepare_chron_control_tables.R`](#prepare_chron_control_tables.r) - prepare the chronology tables for age-depth modelling
- [`02_Run_age_depth_models.R`](#run_age_depth_models.r)- create age-depth models with BChron
- [`03_Predict_ages.R`](#predict_ages.r) - estimate the ages of individual levels
- [`04_Save_AD_figures.R`](#save_ad_figures.r) - save visual output of the age-depth models in pdf format
- [`05_Merge_chron_output.R`](#merge_chron_output.r) - link age-depth models to corresponding datasets

#### Note on age-depth modelling
...
Although F14C has been recommended by experts for the reporting of post-bomb samples ([Reimer et al. 2004](https://doi.org/10.1017/S0033822200033154), in practice, the bulk of the reporting is done as modern radiocarbon dates followed by pMC to a much lesser extent. The Workflow currently does not deal with F14C as no such cases have been detected to date. As this is not consistent with the rest of the data, back-transformation could be done when working with other data sources, and the {[IntCal](https://cran.r-project.org/web/packages/IntCal/index.html)} package can be used to do so.

#### *02_Run_age_depth_models.R*

Individual age-depth models are estimated using the {[Bchron](http://andrewcparnell.github.io/Bchron/)} package, which estimates the Bayesian probabilistic trajectory of the age-depth model curve (non-parametric chronology model to age-depth data according to the Compound Poisson-Gamma model). Therefore, it is suitable for various combinations of control point types, outliers, and age reversals.
...
### [V. Harmonisation]{.text-background-pink .bold}: `/05_Harmonisation/`

- `Run_01_05.R` - run all scripts within this folder
- [`01_Harmonisation.R`](#harmonisation.r) -  prepare all harmonisation tables and harmonise the raw counts.

#### Note on harmonisation

The goal of taxonomic harmonisation is to standardise all site-level names to the same pollen morphotypes (set of pollen and spore morphotypes used for all pollen records) and thus reduce the effect of taxonomic uncertainty and nomenclatural complexity (See relevant literature in Appendix 1 of [Flantua et al. 2023](https://doi.org/10.1111/geb.13693)). For this purpose, a *harmonisation table* can be created that groups the morphotypes into the highest taxonomic level that is most likely to be identified by most of the pollen analysts.
...
### [VI. Data filtering]{.text-background-coral .bold}: `/06_Main_filtering/`
...
The number of counted pollen grains at each level is an index of data quality. To obtain a reliable representation of the vegetation, researchers often aim to count more than `300` pollen grains (following Moore et al., 1991), but other recommendations may also have been followed (>`150`; e.g. [Djamali & Cilleros, 2020](https://doi.org/10.1016/j.revpalbo.2019.104156)) and will vary with region and by scientific question ([Birks & Birks, 1980](https://www.google.cz/books/edition/_/9wrDQgAACAAJ?hl=en&sa=X&ved=2ahUKEwiLnKaxsISMAxUb0gIHHfS_FjsQ8fIDegQIHxAJ)). For example, to achieve a representative sample of the regional pollen pool, counts in Arctic records may only reach c. `100` grains per level, whereas counts in Mediterranean sites can be as high as `1000` ([Birks & Birks, 1980](https://www.google.cz/books/edition/_/9wrDQgAACAAJ?hl=en&sa=X&ved=2ahUKEwiLnKaxsISMAxUb0gIHHfS_FjsQ8fIDegQIHxAJ), p. 165), but the main determinant can also be the preference of the pollen analyst. Reasons for low numbers (<`100`) are often mainly due to time constraints of the data contributor, but can also be natural depositional phenomena causing poor pollen preservation, such as low local pollen production in arctic or alpine environments. Given that statistical inferential power is proportional to sample size, we recommend defining a minimum number of total pollen grains in each level. Subsequently, whole records can be selected on the proportion of levels with a selected minimum number of pollen grains counted per level.  
...
### [VII. Outputs]{.text-background-orange .bold}: `/07_Outputs/`

- `Run_01_07.R` - run all scripts within this folder
- [`01_Pollen_diagrams.R`](#pollen_diagrams.r) - save pollen diagrams for all records
- [`02_Save_assembly.R`](#save_assembly.r) - save data assembly with selected variables (columns)
- [`03_Save_references.R`](#save_references.r) - save reference and metatable

#### *01_Pollen_diagrams.R*

Pollen diagrams for all records will be created using the {[rioja](https://cran.r-project.org/web/packages/rioja/rioja.pdf)} package. The harmonised data will be automatically transformed into relative proportion for plotting purposes. The pollen diagrams will be saved in the `/Outputs/Figures/Pollen_diagrams/` folder and split into sub-folders defined by *Region* (e.g., continent).
...
A ready-to-use, taxonomically harmonised and standardised compilation of fossil pollen data (ready for the analytical stage) is produced and saved in the `/Outputs/Data/` folder. The file is saved as a *tibble* (by {[tibble](https://tibble.tidyverse.org/)} package from [tidyverse](https://www.tidyverse.org/)) in `rds` format.
