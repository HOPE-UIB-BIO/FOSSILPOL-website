...
3. **Salida del flujo de trabajo** (`Outputs/`, ver [Sección VII](#vii.-salidasoutputs-07_outputs) para más información):
...
- [`01_Download_neotoma.R`](#download_neotoma.r) - descarga los datos de polen desde la base de datos Neotoma
- [`02_Extract_samples.R`](#extract_samples.r) - crea una tabla a partir de las listas descargadas de Neotoma y descarga la información de los autores
- [`03_Filter_dep_env.R`](#filter_dep_env.r) - obtiene los datos del ambiente de depósito y filtra registros según las preferencias del usuario
- [`04_Extract_chron_control_tables.R`](#extract_chron_control_tables.r) - obtiene las cronologías, incluyendo la tabla preferida con los puntos de control de cronología
- [`05_Extract_raw_pollen_data.R`](#extract_raw_pollen_data.r) - extrae los conteos de polen brutos de Neotoma y filtra por grupos ecológicos seleccionados por el usuario.
...
2. Los datos se extraen y formatean para ser compatibles con los datos de Neotoma utilizando la función `RFossilpol::import_datasets_from_folder()`, con los siguientes argumentos:
   ...
   - `source_of_data` - marcará la fuente de cada conjunto de datos en la compilación en el resumen de metadatos (ver [sección VII](#vii.-salidasoutputs-07_outputs)). Por defecto se define como `"personal_data"`
   - `data_publicity` - marcará la publicidad de los datos de cada conjunto en la compilación en el resumen de metadatos (ver [sección VII - Outputs](#vii.-salidasoutputs-07_outputs)). Por defecto se define como `"restricted"`
...
3. Los nombres de los contribuyentes de datos se extraen y se añaden a la [base de datos Autor-Dataset](#extract_samples.r) usada para la atribución de autor-dataset (ver [sección VII - Outputs](#vii.-salidasoutputs-07_outputs)).

4. Los datos se tratan de manera similar a los datos de Neotoma, en cuanto al filtrado por ubicación geográfica, número de niveles ([Fig. 2](#figura-2-figura-2) - criterios config **5**, **6**), y ambientes de depósito (punto [***stop-check***](#data-storage), [Fig. 2](#figura-2-figura-2)).
...
Existe la posibilidad de que algunos conjuntos de datos de otras fuentes ya estén en Neotoma. Para evitar duplicados en la compilación final, el flujo de trabajo comparará conjuntos de ambas fuentes e identificará posibles duplicados. Este paso es opcional, pero se recomienda seguirlo. Para ello, el usuario debe especificar `detect_duplicates` == `TRUE` en el archivo de configuración (esto es predeterminado, [Fig. 2](#figura-2-figura-2) - criterio config **7**). El flujo de trabajo iniciará una subrutina ...
...
1. Todos los nombres de taxones se transforman en un formato más fácil de manejar por ordenador para su manipulación. La función `RFossilpol::proc_clean_count_names()` primero transforma los caracteres especiales a texto (por ejemplo, `+` a `_plus_`) y luego utiliza {[janitor](https://sfirke.github.io/janitor/)} para transformar en el estilo ["*snake_case*"](https://en.wikipedia.org/wiki/Snake_case). Además, el usuario puede especificar cambios adicionales mediante el argumento `user_name_patterns` (ver ejem...
...
3. Se asigna información espacial a cada registro según los shapefiles geográficos proporcionados. Específicamente:
    ...
    - **Información de región** - el shapefile en `Data/Input/Spatial/Regions_shapefile` asignará los nombres regionales a cada registro (ver [Sección Entrada de datos](#data-input)). El usuario puede (y se recomienda) cambiar la delimitación espacial alterando o reemplazando el shapefile.
  
    - **Delimitación política (países)** - obtenido de la base de datos [GADM](www.gadm.org), versión 2.8, noviembre 2015.
  
    - **Región de armonización** - el shapefile en `Data/Input/Spatial/Harmonisation_regions_shapefile` asignará la región de armonización (para vincular la tabla de armonización correspondiente; ver [Sección Entrada de datos](#data-input)). El shapefile predeterminado en el flujo de trabajo es una copia del shapefile de información regional, pero debe ajustarse para corresponder al área cubierta por las distintas tablas de armonización.
  
    - **Curvas de calibración (normal y post-bomba)** - dependiendo de la posición geográfica del registro, se debe asignar una curva de calibración distinta, ya que se usan curvas diferentes para los hemisferios norte y sur, y para ambientes terrestres y marinos. Ver más detalles sobre [curvas de calibración](#calibration-curves).
...
4. El flujo de trabajo creará una nueva tabla con los límites de edad para cada región representada en los datos, que debe ser editada por el usuario (un punto [***stop-check***](#data-storage), [Fig. 2](#figura-2-figura-2)). Por ejemplo, la tabla `Regional_age_limits` tendrá los siguientes valores:
...
Las curvas de calibración se asignan a cada punto de control según varios criterios. Si un punto de control tiene un tipo marcado para calibrar (ver sección siguiente), la curva se asigna según la posición geográfica del registro. Solo los puntos de control con edades de radiocarbono sin calibrar requieren recalibración. FOSSILPOL incluye un shapefile basado en la figura 7 de [Hogg et al (2020)](https://doi.org/10.1017/RDC.2020.59) para asignar correctamente IntCal20, SHCal20 y una curva mixta. Se sigue la rec...
...
- filtrar tipos de puntos de control no deseados seleccionados por el usuario ([***stop-check***](#data-storage))
...
Para enlazar los nombres con la visualización original de Neotoma, el usuario puede usar la `taxa_reference_table` (ver [proceso de limpieza de nombres](#additional-data-preparation)).
...
- `"reproducibility_bundle"` - un zip (`reproducibility_bundle.zip`) con el archivo de configuración, todas las tablas [***stop-check***](#data-storage), y todos los shapefiles. La idea es aumentar la reproducibilidad; este zip puede compartirse al publicar un proyecto creado con el flujo FOSSILPOL, o durante revisión para obtener feedback detallado antes de la publicación.
